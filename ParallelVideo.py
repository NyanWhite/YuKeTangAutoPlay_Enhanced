...
import threading
import time
from yuketangPlus import YuketangPlayer, load_config
...
def main():
    ...
    multi_thread_per_course = settings.get('multi_thread_per_course', False)  # 新增配置项
    threads_per_course = settings.get('threads_per_course', 3)  # 新增配置项
    
    print(f"配置加载完成，最大并发数: {max_concurrent}, 无头模式: {headless}")
    print(f"单课程多线程: {'启用' if multi_thread_per_course else '禁用'}, 每课程线程数: {threads_per_course}")
    ...
    for course in courses:
        ...
        player = YuketangPlayer(
            course['url'], 
            course['cookie'], 
            headless=headless,
            multi_thread=multi_thread_per_course,  # 新增参数
            max_threads=threads_per_course  # 新增参数
        )
        player.start()
